...
import id.bpdlh.fdb.core.databinding.FdbFourStepperBinding

class FdbFourStepper(context: Context, attributeSet: AttributeSet) :
    FrameLayout(context, attributeSet) {
    private var binding: FdbFourStepperBinding? = null
    private val typedArray: TypedArray =
        context.obtainStyledAttributes(attributeSet, R.styleable.FdbStepper, 0, 0)

    init {
        binding =
            FdbFourStepperBinding.inflate(LayoutInflater.from(context), this, true)
        setDescription(
    ...
    fun setStep(step: Int) {
        binding?.run {
            tvStep.text = context.getString(R.string.text_stepper_four_step, step.toString())
            when (step) {
                1 -> {
